...
const CreateFlashcard = ({ userId, deckId }) => {
    ...
    const handleChange = (event) => {
        ...
        if (validateProperty(event.target.name, event.target.value) === null) {
            const currentValues = formValue
            currentValues[event.target.name] = event.target.value.trim()
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            //if (error = 500) {
            //    alert("Front and Back image have to be valid URL's")
            //}
